...
# Crear una LWPOLYLINE con los puntos del eje
...
# El avance no es uniforme: algunos tramos avanzan más rápido
# Definir una curva de avance por sección del eje (0-300m)
# Días 1-3: zona de corte (primeros 150m del eje avanzan más rápido)
# Días 4-7: zona de relleno (últimos 150m)

def factor_avance_x(x_rel_, dia, n_dias):
    """Factor de avance (0-1) por pixel según posición x y día."""
    progreso_total = dia / n_dias
    # Zona A (0-150m): avanza primero
    zona_a = x_rel_ < 150
    # Zona B (150-300m): avanza después
    zona_b = x_rel_ >= 150
    fac = np.zeros_like(x_rel_, dtype=float)
    # Zona A: avanza a ritmo doble en los primeros días
...
for dia in range(1, N_DIAS+1):
    ...
    # Calcular delta aproximado para el log
...
print(f"""
Listo. Para probar el pipeline completo, corre:
...
# Actualizar config para que apunte al DXF
import json
...
if cfg_path.exists():
    ...
    print("  proyecto_config.json actualizado → eje_via: baseline/eje_via.dxf")
